...
- **Sort transcript segments before processing:** Wholesale segmentation/merging logic must sort raw segments by `start` timestamp first. Out-of-order Whisper output or reordered JSON at rest will cause negative gaps and incorrect merges.
- **Create output dirs in write helpers:** Every writer (`_write_segments`, etc.) should `os.makedirs(output_dir, exist_ok=True)` to match the project convention and avoid FileNotFoundError on fresh runs.
- **Externalize LLM prompts as markdown templates:** Store system prompts, cut/keep criteria, and editorial specs as standalone .md files with explicit substitution markers (e.g., `{{AGGRESSIVENESS}}`) rather than inline Python strings. Inject runtime config values at load time via `str.replace`. Use `json.dumps()` for list/dict values. This separates editorial policy from implementation, makes prompt diffs reviewable in PRs, and allows non-engineers to iterate on instructions without touching source code.
